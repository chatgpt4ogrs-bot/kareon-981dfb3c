...
import { Shield, Search, AlertTriangle, Plus, Minus, Save, RotateCcw, Loader2 } from "lucide-react";
...
const GerenciarPermissoes = () => {
  ...
  const { data: profiles = [], isLoading } = useQuery({
    queryKey: ["perm-profiles", isAdmin, profile?.clinica_id],
    queryFn: async () => {
  ...
  const isRoleAssignable = (role: AppRole) => {
    ...
    return role !== "admin"; // clinica_admin não pode atribuir admin master
  };
      ...
      <div className="flex items-start justify-between flex-wrap gap-3">
        <div>
          <h1 className="text-2xl font-bold text-foreground flex items-center gap-2">
            <Shield className="w-6 h-6" /> Gerenciar permissões
          </h1>
          <p className="text-sm text-muted-foreground">
            Atribua roles aos usuários. As alterações são revisadas antes de salvar.
          </p>
        </div>
        ...
        <Card className="border-amber-300 bg-amber-50/40">
          <CardContent className="p-4 flex items-start gap-3">
            ...
            <div className="text-sm text-foreground">
              Você tem <strong>{staged.length} alteração(ões) pendente(s)</strong>. Revise antes de salvar — mudanças de permissão afetam imediatamente o que cada usuário vê e pode fazer.
            </div>
          </CardContent>
        </Card>
      )}
