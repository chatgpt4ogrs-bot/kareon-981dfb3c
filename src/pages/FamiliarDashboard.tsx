...
const progressoEmoji: Record<string, string> = { regressao: "📉", manteve: "➡️", progresso_leve: "📈", progresso_significativo: "🚀" };
...
function useFamiliarData() {
  ...
  // For familiar, we find patients linked via responsavel_email
  return useQuery({
    queryKey: ["familiar-data", user?.id],
    queryFn: async () => {
      if (!user?.email) return null;

      // Find patients linked to this family member's email
      ...
      if (!pacientes || pacientes.length === 0) return { pacientes: [], sessoes: [], objetivos: [] };

      const pacienteIds = pacientes.map((p) => p.id);

      const [sessoesRes, objetivosRes] = await Promise.all([
...
const FamiliarDashboard = () => {
      ...
      <div className="space-y-6">
        <div>
          <h1 className="text-2xl font-bold text-foreground">
            Olá, {profile?.nome?.split(" ")[0] || "Familiar"} 👋
          </h1>
        ...
        <Card>
          <CardContent className="flex flex-col items-center justify-center py-12">
            ...
            <p className="text-sm text-muted-foreground text-center mt-1">
              Entre em contato com a clínica para vincular o paciente ao seu email.
            </p>
          </CardContent>
        </Card>
      </div>
  ...
  const proximasSessoes = sessoes
    .filter((s) => isFuture(new Date(s.data_hora)) || isToday(new Date(s.data_hora)))
    .slice(0, 5);

  const ultimasSessoes = sessoes
    .filter((s) => !isFuture(new Date(s.data_hora)))
    .slice(0, 5);
  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto">
      ...
      {/* Pacientes cards */}
      {pacientes.map((pac) => {
        const pacObjetivos = objetivos.filter((o) => o.paciente_id === pac.id);
        const pacProgresso = pacObjetivos.length > 0
          ? Math.round(pacObjetivos.reduce((s, o) => s + o.progresso, 0) / pacObjetivos.length)
          : 0;
        const pacSessoes = sessoes.filter((s) => s.paciente_id === pac.id);
          ...
          <Card key={pac.id} className="border-primary/20">
            ...
            <CardContent>
              <div className="grid grid-cols-3 gap-4 mb-4">
                ...
                <div className="text-center p-3 bg-muted/50 rounded-lg">
                  <p className="text-2xl font-bold text-foreground">{pacObjetivos.filter(o => o.status === "em_andamento").length}</p>
                    ...
                    <div key={s.id} className="flex items-center gap-3 p-3 rounded-lg bg-muted/50">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-muted-foreground">
                          {format(new Date(s.data_hora), "dd/MM · HH:mm")}
                        </p>
                      </div>
                    </div>
                  );
        ...
        <Card>
          ...
          <CardContent>
            {objetivosAtivos.length === 0 ? (
              ...
              <div className="space-y-3">
                {objetivosAtivos.slice(0, 5).map((o) => (
      ...
      {/* Últimas sessões - resumo para o familiar */}
        ...
        <Card>
          ...
          <CardContent>
            <p className="text-sm text-muted-foreground mb-3">
              {activeCameras.length} câmera{activeCameras.length !== 1 ? "s" : ""} disponível{activeCameras.length !== 1 ? "is" : ""}
            </p>
            <Link to="/cameras">
              <Button variant="outline" className="gap-2">
                <Camera className="w-4 h-4" /> Ver câmeras
              </Button>
            </Link>
          </CardContent>
        </Card>
